...
@pytest.mark.parametrize(
    "args, expected", [
        ...
        pytest.param(['--no-micromamba', '--no-conda-exe'], '', marks=[pytest.mark.xfail])
    ]
)
def test_ensure_simple(args: List[str], expected: str, docker_client: docker.client.DockerClient, test_container):
    container: docker.models.containers.Container = docker_client.containers.run(
        test_container,
        detach=True,
        command=['ensureconda', *args]
    )
    try:
        res = container.wait()
...
@pytest.mark.parametrize(
    "args, expected", [
        ...
        (["--no-mamba", "--no-micromamba", "--no-conda"], '/root/.local/share/ensure-conda/conda_standalone'),
    ]
)
def test_ensure_full(args: List[str], expected: str, docker_client: docker.client.DockerClient, test_container_full):
    container: docker.models.containers.Container = docker_client.containers.run(
        test_container_full,
        detach=True,
        command=['ensureconda', *args]
